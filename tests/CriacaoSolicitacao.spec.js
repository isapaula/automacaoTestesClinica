...
test('test', async ({ page }) => {
  ...
  await page.waitForTimeout(4000);
  await page.getByRole('textbox', { name: 'Informe a especialidade' }).click();
  await page.getByRole('textbox', { name: 'Informe a especialidade' }).fill('toc, ansiedade');
  await page.locator('#selectHora').selectOption('10:00');
  await page.locator('#observacao').click();
  await page.locator('#observacao').fill('toc, ansiedade');
  await page.getByRole('button', { name: 'Enviar' }).click();
  await page.waitForTimeout(4000);
  await expect(page.getByRole('heading', { name: 'Bem vindo, Paciente!' })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'AGUARDANDO_TRIAGEM' })).toBeVisible();
  await page.waitForTimeout(4000);
});
